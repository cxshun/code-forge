...
- **F3.1.8** **输入边界（MVP）**：支持**纯文本**消息与**引用回复**触发 Agent（引用回复详见 F3.1.10）；图片 / 文件 / 语音等富消息暂不处理（静默忽略或回复"暂不支持该消息类型"）
- **F3.1.9** **流式推送节流**：`text_delta` 按阈值（~500ms 窗口或 ~200 token）合并更新进度卡片，避免触发飞书卡片更新 QPS 限制
- **F3.1.10** **引用回复**：用户引用群里历史消息 + @ 机器人可触发 Agent；接入层提取被引用消息的**纯文本**（优先从推送事件 quote 字段，取不到则以 `parent_id` 调飞书 IM API 拉取），作为「引用块」前置注入到本次 Run 的 user message；**不恢复**被引用消息所属 session 的历史上下文（与 1 Session:1 Run 一致，design D39）；被引用消息为富卡片时尽力提取纯文本，提取失败则标注"（引用的是一条卡片消息）"
...
- 图片 / 文件 / 语音等富消息输入（MVP 仅纯文本 + 引用回复触发，F3.1.8 / F3.1.10）
