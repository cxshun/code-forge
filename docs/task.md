...
> 任务拆分依据：[spec.md](./docs/spec.md)（需求）/ [design.md](./docs/design.md)（设计）/ [api.md](./docs/api.md)（接口）。
> 本文件只拆任务、不开始开发。任务完成时勾选验收标准 `[x]`。
...
> **切片验收点 P4**：群里 @ 机器人能收到 Thinking 反馈并路由到对应 WS（此时还不跑 Agent）。
